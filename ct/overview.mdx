...
<CardGroup cols={2}>
  <Card title="Confidentiality where liquidity already lives" icon="location-dot">
    One-click confidentiality on the chains and apps you already use. No bridging, no extra cost, no privacy loss when moving funds.
  </Card>
  <Card title="Low fees" icon="coins">
    Fees are capped and practical. Lightweight cryptography means far lower gas than heavy ZK proofs or FHE systems.
  </Card>
  <Card title="No black-box trust" icon="shield-halved">
    No single-TEE setups, outsourced ZK provers, or centralized offchain coprocessors. Fully verifiable onchain.
  </Card>
  <Card title="Fast enough for real usage" icon="bolt">
    ZK proofs run in a normal payment flow with no minutes-long waits. Throughput scales with FPGA/GPU acceleration.
  </Card>
</CardGroup>
