...
| **Fairyport Relayer** | Cross-chain transport | Off-chain relayer that moves messages between chains and FairyRing |

At a high level: users deposit tokens into the EVM escrow, receive a confidential balance from FairyRing, perform confidential transactions with that balance, and optionally redeem back to the EVM chain when withdrawing.
